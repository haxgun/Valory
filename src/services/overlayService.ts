...
const getLastFiveMatches = async(): Promise<MatchResult[]> => {
  try {
    const response: AxiosResponse = await apiClient.get(`/v4/by-puuid/matches/${settingsStore.region}/pc/${settingsStore.puuid}?mode=competitive&size=5`);
    ...
    return matches.map((match: any) => {
      const player = match.players.find(
        (p: any) => p.puuid === settingsStore.puuid
      );

      if (!player) {
        console.error(`Player with puuid ${settingsStore.puuid} not found in match ${match.metadata.match_id}`);
        return "Draw";
      }
